document.addEventListener('DOMContentLoaded', () => {
  ...
  function playGame() {
      ...
      gameMenu.classList.add('hidden'); // Oculta o menu do jogo

      const game = document.getElementById('game');
      game.classList.remove('hidden'); // Exibe a tela do jogo

      resetGame(); // Reseta o estado do jogo
      displayCards(); // Exibe as cartas na tela
  ...
  function toggleDifficulty() {
      ...
      gameDifficultyOptionButton.textContent = `Difficulty: ${isHardMode ? 'Hard' : 'Easy'}`; // Atualiza o texto do botão
  }
  ...
  function displayCards() {
      ...
      cardsContainer.innerHTML = ''; // Limpa o contêiner de cartas
      ...
      totalCards = cardSet.length; // Armazena o total de cartas
      ...
      cardSet.forEach((cardData) => {
          ...
          card.addEventListener('click', flipCard); // Adiciona evento de clique na carta
          
          cardsContainer.appendChild(card); // Adiciona a carta ao contêiner
  ...
  function backToMenu() {
      ...
      // Oculta a tela do jogo se estiver visível
      if (!game.classList.contains('hidden'))
          game.classList.add('hidden');

      // Oculta a tela de ranking se estiver visível
  ...
  function saveScore(score) {
      ...
      if (playerName && score !== undefined) {
          ...
          localStorage.setItem('ranking', JSON.stringify(ranking)); // Salva o ranking atualizado no Local Storage
      }
  }
  ...
  function resetGame() {
      ...
      matchedCards = 0; // Reseta o contador de cartas combinadas
      consecutiveMatches = 0; // Reseta a sequência de acertos
      clearInterval(timerInterval); // Para o temporizador
      timeRemaining = isHardMode ? 80 : 60; // Define o tempo restante baseado na dificuldade
      document.getElementById('score').innerText = `Score: ${score}`; // Atualiza o display da pontuação
      document.getElementById('timer').textContent = `Time Remaining: ${timeRemaining}`; // Atualiza o display do temporizador
  }
});
